import random
from collections import defaultdict
COLORS = ['R', 'G', 'B', "Y", 'W', 'O']
TRIES = 10
CODE_LEN = 4


def generate_code():
    code = []

    for _ in range(CODE_LEN):
        color = random.choice(COLORS)
        code.append(color)

    return code

def guess_code():
    while True:
        guess = input("Guess: ").upper().split(" ")

        if len(guess) != CODE_LEN:
            print(f"You must guess {CODE_LEN} colors.")
            continue

        for color in guess:
            if color not in COLORS:
                print(f"Invalid color {color}, try again.")
                break
        else:
            break

    return guess


def check_code(guess_code, real_code):
    color_cnts = defaultdict(int)
    correct_pos = 0
    incorrect_pos = 0

    for color in real_code:
        if color not in color_cnts:
            color_cnts[color] += 1

    for guess_color, real_color in zip(guess_code, real_code):
        if guess_color == real_color:
            correct_pos += 1
            color_cnts[guess_color] -= 1


    for guess_color, real_color in zip(guess_code, real_code):
        if color_cnts[guess_color]:
            incorrect_pos += 1
            color_cnts[guess_color] -= 1


    return correct_pos, incorrect_pos

def game():
    print(f"welcome to mastermind, you have {TRIES} to guess the code...")
    print( "The valid colors are", COLORS)

    code = generate_code()
    print(f"This is the code geenrated from the computer: {code}")
    for attempts in range(1, TRIES+1):
        guess = guess_code()
        correct_pos, incorrect_pos = check_code(guess, code)

        if correct_pos == CODE_LEN:
            print(f"You guessed the code in {attempts} tries!")
        else:
            print(f"Correct position: {correct_pos} | Incorrect Position: {incorrect_pos}")
    else:
        print(f"you ran out of tries, the code was: {code}")


if __name__ == "__main__":
    game()
